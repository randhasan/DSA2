    ...
    if(G == null){
      ...
      if(!G.marked[destination]){
		...
		while (current != source) {
		  path.push(current);
		  current = G.edgeTo[current];
		}
		path.push(source);
		int hops = path.size() - 1;
		System.out.println("The shortest route from " + cityNames[source] + " to " + cityNames[destination] + " has " + hops + " hops:");
		int from = path.pop();
		      
	      while (!path.empty()) {
		      int to = path.peek();
		      System.out.print(cityNames[from] + " -> " + cityNames[to] + " ");
		      from = path.pop();
	      }
	      System.out.println();
    ...
    public void dijkstras(int source, int destination) {
      ...
      while (nMarked < this.v) {
        ...
        //This can be done more effiently using a priority queue!
	      ...
	      if(current == -1) break; // All remaining vertices are disconnected
	      marked[current] = true;
	      nMarked++;
